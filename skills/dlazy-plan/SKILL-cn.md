...
面向 CLI / 外部智能体的视频规划工具。通过 Agent，返回一份逻辑模板（含 map / when 控制结构）和一份已展开的扁平节点列表，后者可直接通过 drawToCanvas 落到画布。
...
执行 `dlazy plan` 命令获取结果。

```bash
dlazy plan -h

Options:
  --prompt [prompt]                    自由文本指令，描述要规划的视频内容（idea + 可选需求 / 规模）。
  --scenario [scenario]                规划场景，决定生成结构。可选：general(通用,默认) / idea2video(完整成片) / single_image(图片) / character_design(角色设计) / storyboard(分镜) / product_showcase(产品展示) / short_video(短视频) / music_video(音乐MV) / tutorial(教程)。 [default: general] (choices: "general", "idea2video", "single_image", "character_design", "storyboard", "product_showcase", "short_video", "music_video", "tutorial")
  --style [style]                      视觉风格枚举。可选：general(通用,默认) / cinematic / anime / pixar_3d / ghibli / watercolor / cyberpunk / vintage_film / documentary / minimalist / cartoon_2d。 [default: general] (choices: "general", "cinematic", "anime", "pixar_3d", "ghibli", "watercolor", "cyberpunk", "vintage_film", "documentary", "minimalist", "cartoon_2d")
  --sessionId [sessionId]              复用已有的 Agent 会话 id（推荐，可命中 prompt cache 并保留上下文）；为空表示新建会话。
